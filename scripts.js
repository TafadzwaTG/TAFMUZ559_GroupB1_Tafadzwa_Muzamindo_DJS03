...
class Booklist extends IBookList {
  ...
  updateShowMoreButton() {
    ...
    document.querySelector("[data-list-button]").innerText =
      `Show more (${remaining})`;
    document.querySelector("[data-list-button]").disabled <= 0;
  }
  handleSeachFormSubmit(event){
    event.preventDefault();
    const formData = new FormData(event.target);
    const filters = Object.fromEntries(formData);
    this.matches = this.books.filter(book => {
        const genreMatch = filters.genre === 'any' || book.genres.includes(filters.genre);
        const titleMatch = filters.title.trim() === '' || book.title.toLowerCase().includes(filters.title.toLocaleLowerCase);
